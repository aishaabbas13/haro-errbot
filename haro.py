...
class Haro(BotPlugin):
    ...
    def docker_start(self, message, service=None):
        ...
        container = client.containers.run(Haro._services[service]['image'],
                                          detach=True,
                                          remove=True,
                                          ports=Haro._services[service]['ports'])
        services = list(self['services'])
        services.append({'id': container.id, 'name': service})
        self['services'] = services
        return Haro._services[service]['info']
